...
class MFABridgeBenchmark {
    ...
    init() throws {
        ...
        guard let queue = device.makeCommandQueue() else {
            throw NSError(domain: "Metal", code: 2, userInfo: [NSLocalizedDescriptionKey: "No command queue"])
        }
    ...
    func runAttentionBridge(
        ...
        guard numHeads == 1 else {
            throw NSError(domain: "MFA", code: 1, userInfo: [NSLocalizedDescriptionKey: "Multi-head not supported"])
        }
        ...
        guard let commandBuffer = commandQueue.makeCommandBuffer() else {
            throw NSError(domain: "Metal", code: 3, userInfo: [NSLocalizedDescriptionKey: "Command buffer creation failed"])
        }
        ...
        guard let encoder = commandBuffer.makeComputeCommandEncoder() else {
            throw NSError(domain: "Metal", code: 4, userInfo: [NSLocalizedDescriptionKey: "Encoder creation failed"])
        }
        ...
        guard let lBuffer = device.makeBuffer(length: lBufferSize, options: .storageModeShared),
              let dBuffer = device.makeBuffer(length: dBufferSize, options: .storageModeShared) else {
            throw NSError(domain: "Metal", code: 5, userInfo: [NSLocalizedDescriptionKey: "L/D buffer allocation failed"])
        }
        ...
        let blockCount = (parallelizationDimension + Int(kernel.blockDimensions.parallelization) - 1) / Int(kernel.blockDimensions.parallelization)
        ...
        if let error = commandBuffer.error {
            throw NSError(domain: "Metal", code: 6, userInfo: [NSLocalizedDescriptionKey: "Execution failed: \(error)"])
        }
    ...
    do {
        ...
        for config in configs {
            ...
            guard let qBuffer = benchmark.createBuffer(sizeBytes: bufferSize),
                  ...
                  let oBuffer = benchmark.createBuffer(sizeBytes: bufferSize) else {
            ...
            for _ in 0..<3 {
                _ = try benchmark.runAttentionBridge(
                    ...
                    numHeads: 1, headDim: UInt16(config.headDim), softmaxScale: 1.0 / sqrt(Float(config.headDim)),
