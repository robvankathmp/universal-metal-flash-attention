...
public class QuantizedAttentionExample {
            ...
            for precision in precisions {
                if let avgTime = results["\(precision)_avg_ms"],
                   let gops = results["\(precision)_gops"] {
                    print("  \(precision): \(String(format: "%.2f ms", avgTime)), \(String(format: "%.1f GOPS", gops))")
                }
            }
    ...
    public func transformerInferenceExample() {
        ...
        for (modelName, hiddenSize, numLayers, numHeads) in modelConfigs {
            ...
            for (name, memory) in memoryEstimates {
                ...
                print("  \(name): \(String(format: "%.1f MB", memory)) (\(String(format: "%.1fx", ratio)) compression)")
            }
            ...
            if let fp16Time = results["FP16_avg_ms"],
               ...
               let int4Time = results["INT4_avg_ms"] {
                ...
                print("  INT8: \(String(format: "%.1f ms", totalINT8Time)) total (\(String(format: "%.1fx", totalFP16Time/totalINT8Time)) faster)")
                print("  INT4: \(String(format: "%.1f ms", totalINT4Time)) total (\(String(format: "%.1fx", totalFP16Time/totalINT4Time)) faster)")
            }
        }
    }
    ...
    private func testAccuracy(tensors: (query: QuantizedTensor, key: QuantizedTensor, value: QuantizedTensor),
