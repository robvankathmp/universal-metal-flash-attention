...
class QuantizedBackwardBenchmark {
  ...
  private func benchmarkBackwardPass(M: Int, N: Int, K: Int) throws {
    ...
    print(
      "  FP16 backward:        \(String(format: "%.3f", fp16Time))ms (\(String(format: "%.1f", fp16Performance)) GFLOPS)"
    )
    print(
      "  Quantized backward:   \(String(format: "%.3f", quantizedTime))ms (\(String(format: "%.1f", quantizedPerformance)) GFLOPS)"
    )
    ...
    let quantizedMemory = M * N * 4 + M * K * 1 + K * N *
      1 // Mixed: FP32 gradients, INT8 weights
    ...
    -> Double
  {
    ...
    var times: [Double] = []
    for _ in 0..<10 {
      let time = try runSingleIteration(
        pipelineState: pipelineState,
        buffers: buffers,
        M: M,
        N: N,
  ...
  private func fillBuffersWithTestData(
    buffers: [MTLBuffer],
    elementSizes: [Int],
    M _: Int,
    N _: Int,
