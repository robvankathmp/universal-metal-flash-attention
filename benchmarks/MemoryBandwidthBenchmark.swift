...
class MemoryBandwidthBenchmark {
    ...
    private func benchmarkSize(M: Int, N: Int, K: Int) throws {
        ...
        print("  BF16 baseline:     \(String(format: "%.3f", bf16Time))ms (\(String(format: "%.1f", bandwidth/bf16Time*1000)) GB/s)")
        print("  INT8 current:      \(String(format: "%.3f", int8CurrentTime))ms (\(String(format: "%.1f", bandwidth/int8CurrentTime*1000)) GB/s)")
        print("  INT8 optimized:    \(String(format: "%.3f", int8OptimizedTime))ms (\(String(format: "%.1f", bandwidth/int8OptimizedTime*1000)) GB/s)")
        print("  Current slowdown:  \(String(format: "%.2f", int8CurrentTime/bf16Time))x")
        print("  Optimized speedup: \(String(format: "%.2f", int8CurrentTime/int8OptimizedTime))x")
    }

    private func measureMemoryLoad(kernel: String, M: Int, N: Int, K: Int, elementSize: Int) throws -> Double {
        ...
        // Warmup runs
        for _ in 0..<3 {
            _ = try runKernel(pipelineState: pipelineState, inputBuffer: inputBuffer, outputBuffer: outputBuffer, M: M, N: N, K: K)
        }

        // Benchmark runs
        var times: [Double] = []
        for _ in 0..<10 {
            let time = try runKernel(pipelineState: pipelineState, inputBuffer: inputBuffer, outputBuffer: outputBuffer, M: M, N: N, K: K)
            times.append(time)
        }
    ...
    private func runKernel(pipelineState: MTLComputePipelineState, inputBuffer: MTLBuffer, outputBuffer: MTLBuffer, M: Int, N: Int, K: Int) throws -> Double {
